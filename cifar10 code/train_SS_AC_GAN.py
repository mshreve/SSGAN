...
gw  = gifn((nz+1*ny, ngf*8*4*4/2), 'gw'); gg = gain_ifn((ngf*8*4*4/2), 'gg')
gb = bias_ifn((ngf*8*4*4/2), 'gb'); gw2 = gifn((ngf*8*2, ngf*4*2, 5, 5), 'gw2'); gg2 = gain_ifn((ngf*4*2), 'gg2')
gb2 = bias_ifn((ngf*4*2), 'gb2'); gw3 = gifn((ngf*4*2, ngf*2*2, 5, 5), 'gw3'); gg3 = gain_ifn((ngf*2*2), 'gg3')
gb3 = bias_ifn((ngf*2*2), 'gb3'); gw4 = gifn((ngf*2*2, ngf*2, 5, 5), 'gw4'); gg4 = gain_ifn((ngf*2), 'gg4')
gb4 = bias_ifn((ngf*2), 'gb4'); gwx = gifn((ngf*2, nc, 5, 5), 'gwx')
...
dwy = difn((ndf*8*4*1, 1), 'dwy'); dwq = difn((ndf*8*4*1, ny), 'dwq')
...
def discrim(X, w, w2, g2, b2, w3, g3, b3, w4, g4, b4, wy, wq):
    ...
    h4f = T.flatten(h4, 2)
    p = sigmoid(T.dot(h4f,wy))
    q = softmax(T.dot(h4f,wq))
    return p, q
...
cost = [g_cost, d_cost, g_cost_s, g_cost_u, d_cost_real_u, d_cost_real_l, d_cost_gen_u, g_cost_s]
...
sample_zmb_0 = floatX(np_rng.uniform(-1., 1., size=(ny, nz))); sample_zmb_0 = np.tile(sample_zmb_0,[nv,1])
